        ...
        if user.status == "InSearch":
            ...
            for searched_user in variables.users_search_queue:
                if searched_user.user_id != user.user_id and searched_user.user_id not in user.denied_partners \
                        and searched_user.status == "InSearch":
                    partners_value = 0
                    ...
                    if partners_value >= 2:
                        ...
                        if partners_value >= 2 and users_value >= 2:
                            ...
                            user.status = "FoundPartner"
                            user.partner_id = searched_user.user_id
                            searched_user.status = "FoundPartner"
                            searched_user.partner_id = user.user_id
